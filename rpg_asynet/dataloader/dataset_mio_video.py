...
#    def __init__(self, root, object_classes, height, width, nr_events_window=-1, augmentation=False, mode='training', #parametros no por defecto????
#                 event_representation='histogram', shuffle=True):
#        """
...
#        return bounding_box
    ...
    def __init__(self, root, object_classes, height, width, nr_events_window=-1, augmentation=False, mode='testing', #parametros no por defecto????
                 event_representation='histogram', shuffle=True):
        """
        Creates an iterator over the N_Caltech101 object recognition dataset.
        ...
        """
        if object_classes == 'all':
            self.object_classes = listdir("data/mis_videos")
        else:
            self.object_classes = object_classes
        self.object_classes.sort()
        ...
        if shuffle:
            zipped_lists = list(zip(self.files,  self.class_labels))
            random.shuffle(zipped_lists)
            self.files,  self.class_labels = zip(*zipped_lists)
...
#        training_ratio = 0.7
        ...
        for i_class, object_class in enumerate(self.object_classes):
            ...
            dir_path = os.path.join("data/mis_videos", object_class)
            image_files = listdir(dir_path)
            nr_samples = len(image_files)
            ...
            if self.mode == 'testing':
                start_idx = 0
                end_idx = start_idx + nr_samples_test
            ...
            for idx in random_permutation[start_idx:end_idx]:
                filepath = os.path.join("data/mis_videos", object_class, image_files[idx])
                if len(filepath.split('0')) == 1 and len(filepath.split('1')) == 1 and len(filepath.split('2')) == 1 and len(filepath.split('3')) == 1 and len(filepath.split('4')) == 1 and len(filepath.split('5')) == 1 and len(filepath.split('6')) == 1 and len(filepath.split('7')) == 1 and len(filepath.split('8')) == 1 and len(filepath.split('9')) == 1:
                    newFiles = self.aniadirFiles(filepath)
                    for archivoNuevo in newFiles:
                        self.files.append(archivoNuevo)
                        self.class_labels.append(i_class)
...
#        all_x = raw_data[0::5]
#        all_p = (raw_data[2::5] & 128) >> 7  # bit 7
#        all_ts = ((raw_data[2::5] & 127) << 16) | (raw_data[3::5] << 8) | (raw_data[4::5]) #de 4 en 4 
#
#        all_p = all_p.astype(np.float64)
#        all_p[all_p == 0] = -1
#
...
#    def aniadirFiles(self, filepath):
#        
#        archivosDevolver = []
#        numeroArchivo = 1
#        tiempoInicioActual = 0.0
#        intervalo = 0.07
#        
#        while True:
#            
...
#                        
#                    if i >= 6: #las 6 primeras son cabecera
#        
#                        data = line.split() #obtenemos los elementos separados y los pasamos al tipo de dato que necesitamos
#                        if float(data[0]) > tiempoInicioActual and float(data[0]) < (tiempoInicioActual+intervalo):
#                            archivo.append(line)
#                        elif float(data[0]) > (tiempoInicioActual+intervalo):
#                            break
#            
#            if len(archivo) > 0:
#                filepathParcial = filepath.split('.')
#                num = str(numeroArchivo)
#                if len(num) == 1:
#                    num = '000' + num
#                elif len(num) == 2:
#                    num = '00' + num
#                elif len(num) == 3:
#                    num = '0' + num
#                filepath1 = filepathParcial[0] + '_' + num + '.txt'
#                if os.path.exists(filepath1):
#                    os.remove(filepath1)
#                fic = open(filepath1, 'w')
#                fic.writelines('%s' % s for s in cabecera)
#                fic.writelines('%s' % s for s in archivo)
#                fic.close()
#            else:
#                break
#            
#            archivosDevolver.append(filepath1)
#            numeroArchivo += 1
#            tiempoInicioActual += intervalo
#        
#        return archivosDevolver
    ...
    def aniadirFiles(self, filepath):
        ...
        with open(filepath, 'r') as f:
            for i, line in enumerate(f.readlines()):                     
                if i >= 6: #las 6 primeras son cabecera
                    data = line.split() #obtenemos los elementos separados y los pasamos al tipo de dato que necesitamos
                    tiempoTotal = float(data[0])
        
        tiempoTotal = math.ceil(tiempoTotal)
        
        archivos =[]
        for i in range(math.ceil(tiempoTotal/intervalo)):
            archivo = []
            archivos.append(archivo)
        
        print('tiempo total: {}'.format(tiempoTotal))
        print('tiempo total / intervalo: {}'.format(tiempoTotal/intervalo))
        print('len archivos : {}'.format(len(archivos)))
        cabecera = []
        archivoActual = 0
        ...
        with open(filepath, 'r') as f:
            for i, line in enumerate(f.readlines()): 
                ...
                if i >= 6: #las 6 primeras son cabecera
                    ...
                    data = line.split() #obtenemos los elementos separados y los pasamos al tipo de dato que necesitamos
                    if float(data[0]) > tiempoInicioActual and float(data[0]) < (tiempoInicioActual+intervalo):
                        archivos[archivoActual].append(line)
                    elif float(data[0]) > (tiempoInicioActual+intervalo):
                        archivoActual += 1
                        tiempoInicioActual += intervalo
                        archivos[archivoActual].append(line)
        
        for numeroArchivo,archivo in enumerate(archivos, start=1):
            if len(archivo) > 0:
                filepathParcial = filepath.split('.')
                num = str(numeroArchivo)
                if len(num) == 1:
        ...
        with open(file_name, 'r') as f:
            ...
            for i, line in enumerate(f.readlines()): 
                ...
                if i >= 6: #las 6 primeras son cabecera
                    ...
                    data = line.split() #obtenemos los elementos separados y los pasamos al tipo de dato que necesitamos
#                    all_ts.append(np.uint32(float(data[0])*1000000))
#                    if float(data[0]) > 2.0 and float(data[0]) < 2.07:
                    all_ts.append(np.uint32(float(data[0])))
                    all_x.append(np.uint32(data[1]))
                    all_y.append(np.uint32(data[2]))
                    all_p.append(np.uint32(data[3]))
                    
#                    if float(data[0]) > 1:
#                        break
                    
        all_x = np.uint32(all_x)
        all_y = np.uint32(all_y)
        all_p = np.uint32(all_p)
        all_ts = np.uint32(all_ts)
...
#        training_ratio = 0.7
        ...
        for i_class, object_class in enumerate(self.object_classes):
            ...
            dir_path = os.path.join(self.root, object_class)
            image_files = listdir(dir_path)
            nr_samples = len(image_files)
            ...
            for idx in random_permutation[start_idx:end_idx]:
                self.files.append(os.path.join(self.root, object_class, image_files[idx]))
                self.class_labels.append(i_class)
...
#        all_y = raw_data[1::4]
#        all_x = raw_data[0::4]
#        all_p = (raw_data[2::4] & 128) >> 7  # bit 7
#        all_ts = ((raw_data[2::4] & 127) << 16) | (raw_data[3::4] << 8) | (raw_data[4::4]) #de 4 en 4 
#
#        all_p = all_p.astype(np.float64)
#        all_p[all_p == 0] = -1
#
