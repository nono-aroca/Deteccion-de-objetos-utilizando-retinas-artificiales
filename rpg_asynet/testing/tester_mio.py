...
import dataloader.dataset_mio_video
from dataloader.loader_mio import Loader
...
class AbstractTrainer(abc.ABC):
    def __init__(self, settings):
        ...
        self.dataset_builder = dataloader.dataset_mio_video.getDataloader(self.settings.dataset_name)
...
class SparseObjectDetModel(AbstractTrainer):
    ...
    def loadPretrainedWeights(self):
        ...
        print(self.settings.pretrained_sparse_vgg)
...
#        test_images = np.zeros([2, int(self.model_input_size[0]*1.5), int(self.model_input_size[1]*1.5), 3])
        ...
        for i_batch, sample_batched in enumerate(self.test_loader):
            event, histogram = sample_batched
...
#            if self.val_batch_step % (self.nr_val_epochs - 2) == 0:
            ...
            if len(num) == 1:
                ...
                num = '0' + num
            file_path = os.path.join(self.settings.vis_dir, 'image_' + num + '.png')
            visualizations.visualizeLocations(vis_locations.cpu().int().numpy(), self.model_input_size,
                                              ...
                                              bounding_box=vis_detected_bbox[:, :-1],
...
#            test_images[int(self.test_batch_step // (self.nr_test_epochs - 2))] = image
            #hasta qui
            self.pbar.update(1)
