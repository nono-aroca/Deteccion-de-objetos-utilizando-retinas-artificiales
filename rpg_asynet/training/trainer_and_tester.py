...
#            self.loadPretrainedWeights()
            self.loadCheckpoint(self.settings.pretrained_sparse_vgg)
    ...
    def loadPretrainedWeights2(self):
        """Loads pretrained model weights"""
        print('\n------------------------------------\n')
        print('\n-------------AQUIIIII22222222---------------\n')
        print('\n------------------------------------\n')
        contenido = os.listdir(self.settings.ckpt_dir)
        file_path = os.path.join(self.settings.ckpt_dir, contenido[len(contenido)-1])
        print(contenido)
        print(file_path)
        self.loadCheckpointTest(file_path)
...
#        self.model.load_state_dict(pretrained_dict_short, strict=False)
    ...
    def train(self):
        ...
        iteracionActual=0
        print('200')
        while iteracionActual < 1:#True:
            iteracionActual += 1
            print('\nestamos en la it',iteracionActual)
            self.trainEpoch()
...
#        self.saveCheckpoint()
    ...
    def test(self):
        ...
        loss_function = yoloLoss
        # Images are upsampled for visualization
#        test_images = np.zeros([2, int(self.model_input_size[0]*1.5), int(self.model_input_size[1]*1.5), 3])
        test_images = []
...
#            if self.val_batch_step % (self.nr_val_epochs - 2) == 0:
            ...
            # Visualization self.batch_step
            file_path = os.path.join(self.settings.vis_dir, 'image_' + str(self.test_batch_step) + '.png')
            visualizations.visualizeLocations(vis_locations.cpu().int().numpy(), self.model_input_size,
                                              features=features.cpu().numpy(), path_name=file_path,
                                              bounding_box=bounding_box[0, :, :].cpu().numpy(),
                                              class_name=[self.object_classes[i] for i in bounding_box[0, :, -1]])
...
#            test_images.append(image)
            path_name = os.path.join(self.settings.vis_dir, 'image_' + str(self.test_batch_step) + '_1.png')
            fig, ax = plt.subplots()
...
#            path_name = os.path.join(self.settings.vis_dir, 'image_' + str(i) + '_1.png')
#            fig, ax = plt.subplots()
#            ax.imshow(test_images[i].astype(np.float))
#            ax.axis('off')
#            fig.savefig(path_name)
#            plt.close()
#        self.writer.add_image('Testing/Input Histogram', test_images, self.epoch_step, dataformats='NHWC')
